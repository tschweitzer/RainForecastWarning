...
| D-22 | The slider spans **−48 h … +2 h** — everything DWD retains (measured: 47 h 55 min, `DWD_RV_FORMAT.md` §3), not a round number chosen for comfort. Past frames are the **t+0 analysis frame of each past cycle**; future frames are leads 1…24 of the **latest** cycle | Still one DWD product (RV); the past is what the radar saw, not a re-forecast. Revised 2026-09-18 from −12 h: the extra 36 h costs ~230 MB of archives and nothing else, and a shorter window discards history that is free to have. `?hours=6` narrows it for anyone who wants a slider they can aim |
