...
| D-26 | Every link we send carries its token in the **URL fragment**, never the query string | A fragment is not sent to the server, so it cannot reach a request log, a proxy history or a `Referer` - which is exactly the leak F-4/F-8 describe for `?token=`. `/confirm` and `/unsubscribe` have now migrated to this shape, and the signup QR with them (D-31). The one remaining exception is forced and documented: D-33 |
...
| D-33 | The **`List-Unsubscribe` header keeps its token in the query string**, alone among the links we send | RFC 8058 has the mail client POST the URI with a body it fixes itself (`List-Unsubscribe=One-Click`) and never run the page, so a fragment cannot be read and the URI is the only place the identity can live. The link a *person* clicks, in the body, is the fragment form; these are now two different URLs rather than one string used twice, and `NtfyNotifier` matches on the label rather than the URL when deciding whether the body already offers an unsubscribe link, so the logged shape cannot leak into a push. **Note that the header is currently advertised and non-functional**: `POST /unsubscribe` reads its token with `Form(...)`, which does not see a query parameter, so a conforming one-click request is answered 400 (measured). Q-13 |
...
| `GET` | `/confirm#t=…` | confirm token | Renders a button and changes nothing; the `POST` behind it activates the subscription, issues the long-lived `api` token and opens a settings session (D-34). Single use, 24 h expiry. The page reads the token from the fragment; `?token=` is still accepted as the hidden field's default so links sent before D-26 keep working. |
...
| `GET` | `/unsubscribe#t=…` | unsubscribe token | Renders a button and changes nothing; the `POST` behind it deletes. The `List-Unsubscribe` header points at `?token=` instead, which D-33 explains. |
...
| Q-11 | Does the `ntfy://` deep link behave on **iOS** as it does on Android? ntfy's docs describe the subscribe-on-open behaviour for the Android app and say nothing about iOS. The web link below it covers the failure either way, so this is a "check on a real iPhone", not a blocker | before telling anyone it is one tap |
...
| Q-13 | **RFC 8058 one-click unsubscribe is advertised but does not work.** `List-Unsubscribe-Post` promises a mail client it can `POST` the `List-Unsubscribe` URI; the handler reads its token with `Form(...)`, which does not see a query parameter, so a conforming request is answered 400 while the form on the page returns 200 and deletes (both measured). Two ways out, and they are a product call rather than a bug fix: make it work by reading the token from the query on `POST`, which newly exposes deletion to mail-client automation - the F-4 worry one level up - or drop `List-Unsubscribe-Post` and keep a plain `List-Unsubscribe` link, losing the bulk-sender deliverability box that Gmail and Yahoo tick above 5 000 messages a day. Nothing is sending bulk mail yet, so neither is urgent; what is not acceptable is leaving a promise in the headers that the server does not keep | when email becomes a real channel, with Q-1 |
